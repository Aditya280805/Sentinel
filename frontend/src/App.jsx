...
// CONFIGURATION
// ========================================================
const API_BASE = ''; // Empty string so it fetches from relative public folder (Vercel)
...
const PriceChart = ({ ticker }) => {
    ...
    useEffect(() => {
        let active = true;
        fetch(`${API_BASE}/api/stock/${ticker}/history.json`)
            .then(r => r.json())
            .then(data => {
...
const App = () => {
    ...
    useEffect(() => {
        fetch(`${API_BASE}/api/stocks.json`)
            .then(res => res.json())
            .then(data => {
    ...
    // Export to CSV
    const exportToCSV = () => {
        const headers = ["Ticker", "Sector", "Price", "CAGR %", "P/E Ratio", "Market Cap", "AI Sentiment"];
        ...
        link.setAttribute("download", "sentinel_stocks.csv");
